...
from info.models import Dept, Class, Course, User, Student, Teacher, Assign, AssignTime, AttendanceTotal, Attendance, AttendanceClass, StudentCourse, Marks, MarksClass
...
class InfoTest(TestCase):
    ...
    def test_index_teacher(self):
        self.client.login(username='test_user', password='test_password')
        dept = self.create_dept(id='CS9')
        s = Teacher.objects.create(user=User.objects.first(), id='test', name='test_name', dept=dept)
        response = self.client.get(reverse('index'))
        self.assertContains(response, s.name)
        self.assertEqual(response.status_code, 200)
    ...
    def test_attendance_view(self):
        ...
        response = self.client.get(reverse('attendance', args=(s.USN,)))
        self.assertEqual(response.status_code, 200)
        self.assertQuerysetEqual(response.context['att_list'], ['<AttendanceTotal: AttendanceTotal object (1)>'], transform=repr)
    ...
    def test_no_attendance__detail(self):
        ...
        self.assertContains(resp, "No attendance records yet.")

    def test_attendance__detail(self):
        s = self.create_student()
        cr = self.create_course()
        t = self.create_teacher()
        ass = Assign.objects.create(class_id=s.class_id, course=cr, teacher=t)
        ac = AttendanceClass.objects.create(assign=ass, date='2024-01-15')
        Attendance.objects.create(student=s, course=cr, attendanceclass=ac, date='2024-01-15')
        self.client.login(username='test_user', password='test_password')
        resp = self.client.get(reverse('attendance_detail', args=(s.USN, cr.id)))
        self.assertEqual(resp.status_code, 200)
        self.assertQuerysetEqual(resp.context['att_list'], ['<Attendance: ' + s.name + ' : ' + cr.shortname + '>'], transform=repr)
