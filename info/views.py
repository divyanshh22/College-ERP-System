...
def index(request):
    if request.user.is_teacher:
        ...
        context = {
            'teacher': teacher,
            'assign_count': teacher.assign_set.count(),
    ...
    if request.user.is_student:
        ...
        att_list = AttendanceTotal.objects.filter(student=student)
        sc_list = StudentCourse.objects.filter(student=student)
        context = {
            'student': student,
            'att_list': att_list,
            'sc_list': sc_list,
            'avg_att': sum(a.attendance for a in att_list) / len(att_list) if att_list.count() else 0,
        }
    ...
    if request.user.is_superuser:
        context = {
            ...
            'assign_count': Assign.objects.count(),
            'students': Student.objects.select_related('class_id', 'class_id__dept').all(),
...
def confirm(request, ass_c_id):
    ...
    for i, s in enumerate(cl.student_set.all()):
        status = request.POST.get(s.USN, 'absent')
        if status == 'present':
            status = 'True'
        else:
            status = 'False'
...
def marks_confirm(request, marks_c_id):
    ...
    for s in cl.student_set.all():
        mark = request.POST.get(s.USN, 0)
        sc, _ = StudentCourse.objects.get_or_create(course=cr, student=s)
        m, _ = sc.marks_set.get_or_create(name=mc.name, defaults={'marks1': mark})
        m.marks1 = mark
        m.save()
    mc.status = True
    mc.save()
...
def edit_marks(request, marks_c_id):
    ...
    for stud in stud_list:
        ...
        m, _ = sc.marks_set.get_or_create(name=mc.name)
        m_list.append(m)
    context = {
        'mc': mc,
        'm_list': m_list,
